 ...
 *
 */
public class BibliographyParserTest {
    public ArrayList<Citation> citations;
    public BibliographyParser bibparser;
    public DocumentPdf document;
    public LoadController load;
    public String bibliography;

    
    public BibliographyParserTest(){
        document = null;
        load = null;
        bibliography = null;
    ...
    public void testGetBibliography() {
        ...
        document = null;
        assertNull(bibparser.getBibliography(document));
        document = load.loadDocument(new File("code/test/testdata/arnold2003otes.pdf"));
        assertEquals(bibliography, bibparser.getBibliography(document));
    }

    @Test
    public void testsplitBibliography(){
        citations = bibparser.splitBibliography(bibliography);
        assertEquals("Angeline, P. J. (1997). Tracking extrema in dynamic environments. In Angeline, P. J. et al., editors, Proc. of the Sixth International Conference on Evolutionary Programming — EP’97, pages 335– 345. Springer Verlag, Heidelberg. 16 Evolutionary Computation Volume x, Number x Optimum Tracking with Evolution Strategies", citations.get(0).getcompleteCit());
        assertEquals("Goldberg, D. E. (1989). Genetic Algorithms in Search, Optimization, and Machine Learning. Addison Wesley, Reading, MA.", citations.get(9).getcompleteCit());
    }
    ...
    public void testparseCitations(){
        ...
        for (Citation cit : citations){
            ...
            System.out.println("authors:"+cit.getAuthors());
            System.out.println("title: "+cit.getTitle());
            System.out.println("^ citation:"+cit.getcompleteCit());
            System.out.println("---");
    ...
    @Test
    public void testsearchForBibliography(){
        DataController data = new DataControllerImpl(new DocumentModelImpl());
        load = new LoadController(data);
        document = load.loadDocument(new File("code/test/testdata/arnold2003otes.pdf"));
        String bibliographie = bibparser.searchForBibliography(document);
        
        citations = bibparser.splitBibliography(bibliographie);
        citations = bibparser.parseCitations(citations);
        System.out.println("----------------------------------------");
        for (Citation cit : citations){
