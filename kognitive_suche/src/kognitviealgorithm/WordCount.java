...
import komplexeSuche.SortTags;
import komplexeSuche.tags;

public class WordCount {
  
  private String text;
  private ArrayList<Keywords> tagFrequency = new ArrayList<Keywords>();   //Datentyp für häufigste Suchwörter
  ...
  for( int i = 0; i < searchText.length; i++){
      ...
      text = searchText[i];
      text = text.replaceAll("[^a-zA-Z0-9 .äöüÄÖÜß?!@]", "");     //hier werden alle Zeichen aus dem Text gelöscht, welche weder Zahlen, Buchstaben, . oder Leerzeichen sind 
  ...
  Collections.sort(tagFrequency, new SortKeywords());
  for(int i = 0; i< 5;i++){
...
private boolean badWord(String string) {
   ...
   */
  String wordList[] ={"der","die","das","dem","den","ein","eines","eine","einem","einen","einer","des","da","wo","von","den","und","dieser","jener","kein","deren",
  ...
  for(int i = 0; i < wordList.length; i++){
    if(string.equalsIgnoreCase(wordList[i])){
      return true;
    }
  }
