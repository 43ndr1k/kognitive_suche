...
public class Pattern {
	...
	private Color[] colors = { COLOR_LIGHTGREEN, COLOR_ORANGE, COLOR_LIGHTBLUE,
			COLOR_RED, COLOR_LIGHTPURPLE, COLOR_LIGHTGREY };
	 ...
	 * @param gui
	 * @param navMode
	 *            Gibt an ob es ein Button nach vorne und hinten geben soll
	 */
	public Pattern(int paneHeight, int paneWidth, String query,
			ReturnTagList tags, GUI gui, Results results, int navMode) {
		...
		visPane = printPattern(padMap, oneHexWidth, columnCorrection,
	...
	public void addTag(double x, double y, String tag) {
		...
		Pane pad = addColorPad(getHexWidth(),
				getColumnCorrection(getHexHeight()), getHexHeight(),
	...
	/**
	 * Berechnet die Anzahl der Spalten in Abhängikeit von der Größe der
	 * Hexagons und des Feldes.
	 * 
	 * @param oneHexWidth
	 *            Breite eines Hexagons
	 * @param columnCorrection
	 *            Versatz zwischen den Hexagons in der Breite
	...
	/**
	 * Berechnet die Anzahl der Reihen in Abhängikeit von der Größe der Hexagons
	 * und des Feldes.
	 * 
	 * @param oneHexHeight
	 *            Höhe eines Hexagons
	 ...
	 * 
	 * @param padMap
	 *            Array das anzeigt, ob dieses Feld aktiv ist.
	 * @param oneHexWidth
	 *            Breite eines Hexagons
	 * @param columnCorrection
	 *            Versatz zwischen den Hexagons in der Breite
	 ...
	 *            Anzahl der Spalten
	 * @param visPane
	 *            Hauptpane auf der die Elemente positioniert werden.
	 * @return visPane mit positionierten Elementen
	 */
	private Pane printPattern(int[][] padMap, double oneHexWidth,
			double columnCorrection, double oneHexHeight, int rows,
			int columns, Pane visPane) {
		...
		for (int y = 0; y < rows; y++) {
			for (int x = 0; x < columns; x++) {
				if ((x % 2) == 0) {
					switch (padMap[x][y]) {
					case 0:
//						visPane = addGreyPad(oneHexWidth, columnCorrection,
//								oneHexHeight, rows, columns, visPane,
//								(x - 0.5), (y - 0.25));
						break;

					case 1:
						visPane = addColorPad(oneHexWidth, columnCorrection,
								oneHexHeight, rows, columns, visPane,
								(x - 0.5), (y - 0.25),
								tags.getTagObject(rangeOfTags - numOfTags)
										.getTag());
						numOfTags--;
						break;

					case 2:
						visPane = addMidPad(oneHexWidth, columnCorrection,
								oneHexHeight, rows, columns, visPane,
								(x - 0.5), (y - 0.25), tags.getSearchword());
						break;

					case 3:
						visPane = addNavPad(oneHexWidth, columnCorrection,
								oneHexHeight, rows, columns, visPane,
								(x - 0.5), (y - 0.25), 1);
...
//								(x - 0.5), (y - 0.75));
	...
	private Pane addMidPad(double oneHexWidth, double columnCorrection,
			double oneHexHeight, int rows, int columns, Pane visPane2,
			double x, double y, String searchWord) {
	...
	private Pane addNavPad(double oneHexWidth, double columnCorrection,
		...
		pad.getLightShape().setOnMouseEntered(
				event -> {
					((Shape) ((StackPane) ((Shape) event.getTarget())
	 ...
	 *            Hauptpane auf der die Elemente positioniert werden.
	 * @param x
	 *            X-Position des Hexagons
	 * @param y
	 *            Y-Position des Hexagons
	 * @return visPane mit neuem aktiven Hexagon
	 */
	@SuppressWarnings({ "restriction", "unused" })
	private Pane addGreyPad(double oneHexWidth, double columnCorrection,
			double oneHexHeight, int rows, int columns, Pane visPane, double x,
			double y) {
		Pad pad;
		StackPane padPane = new StackPane();
		...
		pad = new Pad(PAD_SIZE, (oneHexWidth - columnCorrection + PAD_OFFSET)
	 ...
	 * @return visPane mit neuem aktiven Hexagon
	 */
	private Pane addColorPad(double oneHexWidth, double columnCorrection,
			double oneHexHeight, int rows, int columns, Pane visPane, double x,
		...
		exPadPane.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				/*
				 * addTag(x, y - 1, "TAG!"); Patternpositionen später zum
				 * einkommentieren addTag(x, y + 1, "TAG!"); addTag(x + 1, y -
				 * .5, "TAG!"); addTag(x + 1, y + .5, "TAG!"); addTag(x - 1, y -
				 * .5, "TAG!"); addTag(x - 1, y + .5, "TAG!");
				 * 
				 * addMidPad(oneHexWidth, columnCorrection, oneHexHeight, rows,
				 * columns, visPane, x, y, labelText);
				 */
				//tags.setSearchWord(tags.getSearchword() + " "
				//		+ largeTopicLabel.getText());
				gui.getInstance().setClickedTag(x, y, labelText,
						tags.getSearchword() + " "
								+ largeTopicLabel.getText());
	 ...
	 * Array, welches anzeigt, welche Felder aktiv sind und was in diesem Feld
	 * für ein Pad ist.
	 * 
	 * @param rows
	 *            Anzahl der Reihen
	...
	public void addNewTags(ReturnTagList tags, double x, double y,
			String labelText) {
		activePads = (tags.getSize() < MAX_TAGS - 1) ? tags.getSize()
				: MAX_TAGS - 1;// und das ist shorthand if und else
		...
		addMidPad(getHexWidth(), getColumnCorrection(getHexHeight()),
				getHexHeight(), getRows(getHexHeight()),
				getColumns(getHexWidth(), getColumnCorrection(getHexHeight())),
				visPane, x, y, labelText);
	}
}
